...
/* 원래의 데이터를 buf로 복사 */
int copyImg(BYTE *pImg, BYTE *bufImg)
{
	int i, j, bright;
	bright = 0;
	for (i = 0; i < HEIGHT; i++)
...
// 로봇 찾기
void hBinary(BYTE *pImg, WORD *hImg, BYTE *sImg, int bright)
{
	int i, j, index;
	int threshold1 = 115;
	int threshold2 = 145;
	...
	else if (bright > 90) bright = 15;
	else if (bright > 80) bright = 30;
	else bright = 40;
	...
	for (i = startY - 5; i < endY+5; i++)
	{
		...
		{
			if (hImg[index + j] >= threshold1 && hImg[index + j] <= threshold2 && sImg[index + j] >= 50 && pImg[index + j] >= 50-bright && (j>startX+10 && j<endX+2))
				pImg[index + j] = BLACK;
			else
				pImg[index + j] = WHITE;
		}
...
int outputImg(BYTE *pImg, WORD *outImg)
{
	...
	if (midX < 35) k += 100000;
	else if (midX > 85) k += 200000;
	else  k += 300000;

	if (midY >= 10) k += 1000000;
