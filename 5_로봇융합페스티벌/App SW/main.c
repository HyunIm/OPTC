...
int main(void)
{
	...
	while (1)
	{
		...
		bright = copyImg(i_Img, p_Img);  //p 이진화 영상이 저장될 이미지
		hBinary(p_Img, h_Img, s_Img, bright); //h로 이진화
		...
		printf("밝기 : %d\n크기 : %d\ndistance : %d\ndirection : %d\n", bright, size, distance, direction);
		...
		else
		{
			...
			{
				//1오, 2왼, 3중
				//i- 1정면 2,4,6,7,8,9(왼30-180),  3,5(오), 오뒤(11,10)
				//정면
				if (i % 3 == 1){
					if (size > 5500){
						Send_Play_Motion(21);
			...
			else if ( i%3 != 1 || direction != 3) // 회전
			{
				if (i % 3 == 2){
					...
					if (direction == 1)			Send_Play_Motion(30);
			...
			{
				if (size > 2000 )		{ Send_Play_Motion(33); }
				else if (size > 1000)	{ Send_Play_Motion(15); }
				else					{ Send_Play_Motion(15); }
			}
